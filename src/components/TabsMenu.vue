...
const data = reactive({
    ...
    onTabMenuRemove: (tabItem) => {
        let _tabMenuValue = data.tabsMenuValue;
        let _tabMenuList = data.tabsMenuList;
        if (_tabMenuValue === tabItem) {
            _tabMenuList.forEach((item,index) => {
                if (item.path !== tabItem) return;
                let nextTab = _tabMenuList[index + 1] || _tabMenuList[index - 1];
                if (!nextTab) return;

                _tabMenuValue = nextTab.path;
                router.push(nextTab.path);
            });
        }
        data.tabsMenuValue = _tabMenuValue;
...
const {tabsMenuValue,tabsMenuList,
    onTabMenuClick,onTabMenuRemove,onCloseCurrentTab, onCloseOtherTab,onCloseAllTab} = toRefs(data);
</script>
